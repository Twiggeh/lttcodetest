...
const signifierKeyedGenBlocks = (selectedGenBlocks: SelectedGenBlocks) => {
	...
	const types = ['injection', 'extension'] as const;

	for (const selectedGenBlock of selectedGenBlocks) {
		for (const codeGenBlock of selectedGenBlock) {
			for (const type of types) {
				codeGenBlock[`${type}Code`].forEach(
					(code: typeof type extends 'injection' ? InjectionCode : ExtensionCode) => {
...
const processFiles = (directories: string[]) => {
	...
	const processFile = async (filepath: string) => {
		...
		try {
			...
			const runCodeGen = (file: string, codegenData: typeof codeGenData): string => {
